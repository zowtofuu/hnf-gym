...
<body>
    ...
    <div class="wrapper">
        <h2 class="legend">Subscriptions</h2>
        ...
        <form method="GET" action="ctr_subscriptions.php">
            ...
            <select class="capitalize rounded-sm px8 py16 fv" name="status">
            ...
            <button class="capitalize rounded-sm px8 py16 cursor-pointer btn-primary" type="submit">Apply</button>
            <a class="capitalize rounded-sm px8 py16 btn-anchor btn-secondary" href="ctr_subscriptions.php">Reset</a>
        </form>
        ...
        <div class="capsule-container">
            <div class="capsule">
                <small>Total: <?= htmlspecialchars((string) ($counts['total'] ?? 0)) ?></small>
            </div>
            <div class="capsule">
                <small>Active: <?= htmlspecialchars((string) ($counts['active'] ?? 0)) ?></small>
            </div>
            <div class="capsule">
                <small>Expired: <?= htmlspecialchars((string) ($counts['expired'] ?? 0)) ?></small>
            </div>
            <div class="capsule">
                <small>Suspended:
                    <?= htmlspecialchars((string) ($counts['suspended'] ?? 0)) ?></small>
            </div>
            <div class="capsule">
                <small>Expiring (7 days):
                    <?= htmlspecialchars((string) ($counts['expiring_soon'] ?? 0)) ?></small>
            </div>
        </div>
                        ...
                        <?php foreach ($subscriptions as $row): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <!-- VIEW ID BUTTON -->
                                    <a class="icon-button-plain"
                                        href="ctr_view-subcription-info.php?id=<?= urlencode($row['subscription_id']) ?>"
                                        target="_blank">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
                                            <path
                                    ...
                                    <?php if (($row['status'] ?? '') === 'expired'): ?>
                                        <a class="icon-button-plain"
                                            href="ctr_subscription-renew.php?id=<?= urlencode($row['subscription_id']) ?>">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
                                                <path
